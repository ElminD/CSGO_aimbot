...
namespace offset
{
	...
	constexpr ::std::ptrdiff_t dwGlowObjectManager = 0x531C060;
...
//gets angle from player to enemys head
...
int main()
{
	...
	while (true)
	{
		...
		if (!bestAngle.IsZero())
			memory.Write<Vector3>(clientState + offset::dwClientState_ViewAngles, viewAngles + bestAngle / 1.f); // smoothing
	}
